#Emily Black
#Cleaning 1989 binder data - sybille_springs
...
df_1 <- read.csv('raw_data/binders/sybille_springs_1989_capture_binder_data.csv', 
                 header=FALSE)
...
df_1$num_lice_on_air_sac_right[1] <- 2
df_1$num_hematomas_on_comb_right[15] <- 1
...
previous_strut <- read.csv("prelim_clean/binder_capture_data_1989_prelim_CL_LL_N_VP_MS.csv")
...
#Write to save
write.csv(final_strut,  "prelim_clean/binder_capture_data_1989_prelim_CL_LL_N_VP_MS_SS.csv", 
          row.names=FALSE)
...
final_strut %>%
  ggplot(aes(x=as.numeric(tail_length_mm), color = sex)) +
  geom_histogram() +
  theme_classic()