#Emily Black
#Cleaning 1989 strut data - sybille_springs
...
df_1 <- read.csv('raw_data/binders/sybille_springs_1989_strut_binder_data.csv', 
                 header=FALSE)
...
unique(df_1$lek)
df_1 <- df_1 %>%
  mutate(lek_name= c('sybille_springs'))
df_1 <- df_1 %>%
...
df_wider$date <- gsub(" -", "-", df_wider$date)
...
strut_floppy$left_tag_colour <- toupper(strut_floppy$left_tag_colour)
...
strut_floppy$right_tag_colour <- toupper(strut_floppy$right_tag_colour)
strut_floppy$right_tag_colour <- replace_colors(strut_floppy$right_tag_colour, color_lookup)



#Get stuts for this year and lek - 1990 and sybille and mud springs
leks <- c("sybille_springs")
strut_filtered <- strut_floppy %>%
  filter(year == 1989, lek_name %in% leks) %>%
...
table(df_wider$type)
#there are 56 struts from the floppy, 
#and 73 in our dataframe
...
#correct error in floppies 
# strut_filtered$day <- ifelse(strut_filtered$month == '4' & strut_filtered$day == '1', '2', strut_filtered$day)
...
previous_strut <- read.csv('prelim_clean/1989/merged_struts_binders_floppy_1989_C_MC_NM_SF_LL_N_VP_MS.csv')

#merge the two

final_strut <- rbind(previous_strut, filtered_df_full_join)
unique(final_strut$lek_name)
table(final_strut$source)
table(final_strut$year)
...
#write to computer
write.csv(final_strut, 'prelim_clean/1989/merged_struts_binders_floppy_1989_C_MC_NM_SF_LL_N_VP_MS_SS.csv',
          row.names=FALSE)
