#Emily Black
#Cleaning 1989 strut data - mule_creek
...
df_1 <- read.csv('raw_data/binders/mule_creek_1989_strut_binder_data.csv', 
                 header=FALSE)
...
unique(df_1$lek)
df_1 <- df_1 %>%
  mutate(lek_name= case_when(
    lek=="MCJ" ~ "mule_creek", 
    lek=="NORTH MARSHALL LEK" ~ "north_marshall", 
...
leks <- c("mule_creek", 'north_marshall', 'south_fetterman')
...
table(df_wider$type)
#there are 3 struts from the floppy, 
#and 2 in our dataframe
...
previous_strut <- read.csv('prelim_clean/1989/merged_struts_binders_floppy_1989_C.csv')
...
#write to computer
write.csv(final_strut, 'prelim_clean/1989/merged_struts_binders_floppy_1989_C_MC_NM_SF.csv',
          row.names=FALSE)
