...
df_1 <- read.csv('raw_data/binders/sybille_springs_1990_strut_binder_data.csv', 
                 header=FALSE)
...
df_1 <- df_1 %>%
  mutate(lek_name = case_when(lek == "SYBILLE" ~ "sybille_springs", 
                              lek == "ROUNDTOP" ~ "sybille_springs",
                              lek == "SYBILLE SPRINGS" ~ "sybille_springs",
                              lek == "SYBILLE/ROUNDTOP" ~ "sybille_springs",
                              lek == "MS" ~ "mud_springs",
...
replace_values <- function(column) {
  result <- ifelse(column == "unm", "UM", column)  # Replace "UNMARKED" with "UM"
...
strut_filtered <- strut_floppy %>%
  filter(year == 1990, lek_name %in% leks) %>%
...
table(df_wider$type)
#there are 117 struts from the floppy, 
...
#write to computer
write.csv(final_strut, 'prelim_clean/merged_struts_binders_floppy_1990_MS_N_SF_RS_SS.csv', 
          row.names=FALSE)
