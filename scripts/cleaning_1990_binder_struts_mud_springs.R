...
df_1 <- read.csv('raw_data/binders/mud_spring_1990_strut_binder_data.csv', 
                 header=FALSE)
...
df_1 <- df_1 %>%
  mutate(lek_name = case_when(lek == "MUD SPRINGS" ~ "mud_springs", 
                              lek == "MUD" ~ "mud_springs")) %>%
...
df_1 <- df_1 %>%
  mutate(predator_disturbance_type_ = tolower(case_when(
    predator_disturbance_type_ == "GEagle" ~ "golden eagle", 
...
strut_filtered <- strut_floppy %>%
  filter(year == 1990, lek_name =="mud_springs") %>%
...
strut_filtered <- strut_filtered %>% mutate_all(as.character) %>%
  rename(num_of_struts = struts_5_min)
...
#Need to adjust the 
full_join <- full_join(strut_binders, strut_filtered, by = c('month', 'day', 'time_start', 'left_tag_color', 'right_tag_color'))

#merge columns with commas to see where tags went wrong
for (col in intersect(names(strut_binders), names(strut_filtered))) {
  if (col %in% c('month', 'day', 'time_start', 'left_tag_color', 'right_tag_color')) {
...
join_no_match <- full_join(no_match_binder, no_match_floppy, by = c("month", "day", 'time_start', "num_of_struts"))

for (col in intersect(names(no_match_binder), names(no_match_floppy))) {
  if (col %in% c("month", "day", "time_start", "num_of_struts")) {
    next  # Skip the ID columns
  }
...
#Do again, but with right tag and number of struts
no_match_binder <- join_no_match %>%
  filter(!source=="binder, floppy") %>%
  filter(source=="binder")
no_match_floppy <- join_no_match %>%
  filter(!source=="binder, floppy") %>%
  filter(source=="floppy")
join_no_match_2 <- full_join(no_match_binder, no_match_floppy, by = c('month', 'day', "right_tag_color", "num_of_struts"))

for (col in intersect(names(no_match_binder), names(no_match_floppy))) {
  if (col %in% c('month', 'day', "right_tag_color", "num_of_struts")) {
    next  # Skip the ID columns
  }
  
  join_no_match_2[[col]] <- ifelse(is.na(join_no_match_2[[paste0(col, ".x")]]) & !is.na(join_no_match_2[[paste0(col, ".y")]]),
                                 join_no_match_2[[paste0(col, ".y")]],
                                 ifelse(is.na(join_no_match_2[[paste0(col, ".y")]]) & !is.na(join_no_match_2[[paste0(col, ".x")]]),
                                        join_no_match_2[[paste0(col, ".x")]],
                                        ifelse(join_no_match_2[[paste0(col, ".x")]] == join_no_match_2[[paste0(col, ".y")]],
                                               join_no_match_2[[paste0(col, ".x")]],
                                               paste(join_no_match_2[[paste0(col, ".x")]], join_no_match_2[[paste0(col, ".y")]], sep = ", "))
                                 )
  )
  # Remove the unnecessary columns
  suffixes <- c(".x", ".y")
  cols_to_remove <- paste0(col, suffixes)
  join_no_match_2 <- join_no_match_2[, !(names(join_no_match_2) %in% cols_to_remove)]
}
...
no_match_floppy <- df_full_join %>%
  ...
  filter(source=="floppy") %>%
  filter(type=="strut")
join_no_match <- full_join(no_match_binder, no_match_floppy, by = c("month", "day", 'right_tag_color', "num_of_struts"))
...
write.csv(filtered_df_full_join, 'prelim_clean/merged_struts_binders_floppy_MS.csv', 
          row.names=FALSE)
