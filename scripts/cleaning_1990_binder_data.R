#Emily Black
#Cleaning 1990 binder data - mud springs
...
#Read in raw binder data
mud_springs_1990_capture <- read.csv('raw_data/binders/mud_spring_1990_capture_binder_data.csv', 
                                     header=FALSE)
...
mud_springs_1990_capture<-  fix_column_names(mud_springs_1990_capture)

#Do more general cleanup: 
#Add in NAs so that blanks/empty cells, rows and columns are deleted or standardized 
mud_springs_1990_capture <- mud_springs_1990_capture %>% mutate_all(~ifelse(grepl("^\\s*$", .x), NA, .x))
mud_springs_1990_capture[mud_springs_1990_capture == "N/A"] <- NA
mud_springs_1990_capture <- mud_springs_1990_capture[rowSums(is.na(mud_springs_1990_capture)) != ncol(mud_springs_1990_capture), ]
...
mud_springs_1990_capture$date <- gsub("^\\s*(\\d)", "\\1", mud_springs_1990_capture$date) #remove leading white space from dates
mud_springs_1990_capture$date <- gsub(" ", "-", mud_springs_1990_capture$date)  # Replace spaces with dashes
mud_springs_1990_capture$date <- format(as.Date(mud_springs_1990_capture$date, format = "%d-%m-%Y"), "%d-%m-%Y") #help R read the dates

#Create month day year columns 
mud_springs_1990_capture$year <- format(as.Date(mud_springs_1990_capture$date, format = "%d-%m-%Y"), "%Y") #collect year from date
mud_springs_1990_capture <- mud_springs_1990_capture %>%
  relocate(year, .after = date) #relocate the year column after the date column
mud_springs_1990_capture$month <- format(as.POSIXlt(mud_springs_1990_capture$date, format = "%d-%m-%y"), "%m") #same as above for month
mud_springs_1990_capture <- mud_springs_1990_capture %>%
  relocate(month, .after = year)
mud_springs_1990_capture$day <- format(as.Date(mud_springs_1990_capture$date, format = "%d-%m-%Y"), "%d") #same as above for year
mud_springs_1990_capture <- mud_springs_1990_capture %>%
...
#rename mud springs to match the standardized dataset 
mud_springs_1990_capture <- mud_springs_1990_capture %>%
  mutate(lek_name = case_when(lek == "MUD SPGS" ~ "mud_springs")) %>%
  relocate(lek_name, .after = lek) %>% #relocate the lek column 
  select(-lek) #get rid of the original 

#standardize the age column 
#This is another column that needs to match the standards in others, 
#because it's going to get merged later 
mud_springs_1990_capture$age <- tolower(mud_springs_1990_capture$age) #make lower case
mud_springs_1990_capture <- mud_springs_1990_capture %>%
...
mud_springs_1990_capture$sex <- tolower(mud_springs_1990_capture$sex)
mud_springs_1990_capture$capture_technique <- tolower(mud_springs_1990_capture$capture_technique)


#The full data name is very long - replace with another one temporarily
df_1 <- mud_springs_1990_capture
...
df_1 <- df_1 %>%
  mutate(comb_color = case_when(comb_color == "Olive yell" ~ "olive yellow", 
                                comb_color == "Yell - Olive" ~ "yellow olive", 
                                comb_color == "yellowish/green" ~ "yellowish green", 
                                comb_color == "greenish/yellow" ~ "greenish yellow", 
                                comb_color == "olive yel-yel olive"~ 'olive yellow/yellow olive',
                                comb_color == "Greenish yellow" ~ "greenish yellow", 
         TRUE ~ comb_color), #if not listed above, keep the same 
comb_color = tolower(comb_color)) #all to lower case

#Do the same for air sac colours
unique(df_1$air_sac_color)
df_1 <- df_1 %>%
  mutate(air_sac_color = case_when(air_sac_color == "Bistre yell" ~ "bistre yellow", 
                                air_sac_color == "Chrome yell" ~ "chrome yellow", 
                                TRUE ~ air_sac_color ), 
...
df_2 <- read.csv('raw_data/binders/mud_spring_1990_capture_lab_binder_data.csv', 
                 header=FALSE)
...
df_2 <- df_2 %>%
  mutate(lek_name = case_when(lek_ == "MUD SPRINGS" ~ "mud_springs")) %>%
...
df_2 <- df_2 %>%
  ...
  rename(wygf_leg_band = leg_band__)
...
df_2 <- df_2 %>%
  rename(binder_pdf_scan_page = pdf_page__)
...
#some rows look like df_1 and df_2 had different right or left codes
#These may be typos, or tags that were hard to see in the dark
# #let's fix that using summarize
joined_df <- joined_df %>%
  group_by(left_tag_code) %>%
  summarise(across(everything(), ~ if_else(left_tag_code == "WG1567",
                    toString(na.omit(.x[!is.na(.x)])), .x)), .groups = "drop")
# Define the left_tag_code value of the row to modify
target_row <- "WG1567"
# Modify the specific row within the dataframe
joined_df <- joined_df %>%
  mutate(across(everything(), ~ ifelse(left_tag_code == target_row,
                                       gsub("\\b([\\w\\s]+)(, \\1)\\b", "\\1", ., perl = TRUE),
                                       .)))
...
write.csv(joined_df, "prelim_clean/binder_capture_data_prelim.csv", 
          row.names = FALSE)
