#Emily Black
#Cleaning 1988 binder data - windmill_dam
...
df_1 <- read.csv('raw_data/binders/windmill_dam_1988_capture_binder_data.csv', 
                 header=FALSE)
...
df_1$date <- format(as.Date(df_1$date, format = "%Y-%m-%d"), "%d-%m-%Y") #help R read the dates
...
unique(df_1$lek)
df_1 <- df_1 %>%
  mutate(lek_name = case_when(lek == "WINDMILL DAM" ~ "windmill_dam")) %>%
...
previous_strut <- read.csv("prelim_clean/binder_capture_data_1988_prelim_CL_RP_LL.csv") 
...
#Write to save
write.csv(final_strut,  "prelim_clean/binder_capture_data_1988_prelim_CL_RP_LL_WD.csv", 
          row.names=FALSE)
