#Emily Black
#Cleaning 1988 binder data - mule_creek_junction
...
#read in the mule_creek_junction lek data 
df_1 <- read.csv('raw_data/binders/mule_creek_junction_1988_capture_binder_data.csv', 
                 header=FALSE)
...
unique(df_1$lek)
df_1 <- df_1 %>%
  mutate(lek_name = c("mule_creek_junction")) %>%
...
df_1 <- df_1 %>%
  mutate(age = case_when(age == 'adult' ~ 'adult', 
                         age=='juven' ~ "juven")) #change the "juvenile" code in the data
...
#changed at the end
df_1 <- df_1 %>%
  rename(weight_bird_and_bag_g= weight_bird__bag_g_) %>%
...
#Correct the eighth row
df_1$total_num_lice_on_air_sacs[6] <- 1


df_1 <- df_1 %>% select(-num_lice_on_air_sac_)
...
#Move head length to after num_tail_feathers
df_1 <- df_1 %>%
  relocate(head_length_mm, .after=num_tail_feathers)
...
previous_strut <- read.csv("prelim_clean/binder_capture_data_1988_prelim_CL_RP_LL_WD_MS_N_SS_V_SF.csv") 
...
#remove any bird weights equal to 0
final_strut <- final_strut %>%
  filter(!bird_weight_g=='0')
final_strut$bird_weight_g <- as.numeric(final_strut$bird_weight_g)
final_strut$bird_weight_g <- ifelse(final_strut$bird_weight_g > 100, final_strut$bird_weight_g / 1000, final_strut$bird_weight_g)
final_strut$wing_length_mm <- as.numeric(final_strut$wing_length_mm)
final_strut$wing_length_mm <- ifelse(final_strut$wing_length_mm < 100, final_strut$wing_length_mm *10, final_strut$wing_length_mm)
final_strut$bill_length_mm <- as.numeric(final_strut$bill_length_mm)
final_strut$bill_length_mm <- ifelse(final_strut$bill_length_mm > 100, final_strut$bill_length_mm / 10, final_strut$bill_length_mm)


#Write to save
 write.csv(final_strut,  "prelim_clean/binder_capture_data_1988_prelim_CL_RP_LL_WD_MS_N_SS_V_SF_MCJ.csv", 
           row.names=FALSE)
...
final_strut %>%
  ggplot(aes(x=as.numeric(comb_length_mm), color = sex)) + 
  geom_histogram() +
  theme_classic()