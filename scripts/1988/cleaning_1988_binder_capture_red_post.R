#Emily Black
#Cleaning 1988 binder data - red_post
...
df_1 <- read.csv('raw_data/binders/red_post_1988_capture_binder_data.csv', 
                 header=FALSE)
...
df_1 <- df_1 %>%
  mutate(lek_name = case_when(lek == "RED POST" ~ "red_post")) %>%
...
previous_strut <- read.csv("prelim_clean/binder_capture_data_1988_prelim_CL.csv") 

final_strut <- rbind(previous_strut, df_1)

#Write to save
write.csv(final_strut,  "prelim_clean/binder_capture_data_1988_prelim_CL_RP.csv", 
          row.names=FALSE)
