#Emily Black
#Cleaning 1988 binder data - viewpoint
...
#read in the viewpoint lek data 
df_1 <- read.csv('raw_data/binders/viewpoint_1988_capture_binder_data.csv', 
                 header=FALSE)
...
previous_strut <- read.csv("prelim_clean/binder_capture_data_1988_prelim_CL_RP_LL_WD_MS_N_SS.csv") 

#Rename and add columns
previous_strut <- previous_strut %>%
  rename(total_num_lice_on_air_sacs = num_lice_on_air_sac) %>%
  mutate(num_lice_on_air_sac_right = NA, 
         num_lice_on_air_sac_left = NA, )
...
#Write to save
write.csv(final_strut,  "prelim_clean/binder_capture_data_1988_prelim_CL_RP_LL_WD_MS_N_SS_V.csv", 
          row.names=FALSE)
...
final_strut %>%
  ggplot(aes(x=as.numeric(toe_length_mm), color = sex)) + 
  geom_histogram() +
  theme_classic()