#Emily Black
#Cleaning 1988 strut data - south_fetterman
...
df_1 <- read.csv('raw_data/binders/south_fetterman_1988_strut_binder_data.csv', 
                 header=FALSE)
...
unique(df_1$lek)
df_1 <- df_1 %>%
  mutate(lek_name= c("south_fetterman")
  )
df_1 <- df_1 %>%
...
df_wider$date <- format(as.Date(df_wider$date, format = "%m-%d-%Y"), "%d-%m-%Y") 
...
leks <- c("south_fetterman")
strut_filtered <- strut_floppy %>%
  filter(year == 1988, lek_name %in% leks) %>%
...
table(df_wider$type)
#there are 9 struts from the floppy, 
#and 10 in our dataframe
...
previous_strut <- read.csv('prelim_clean/1988/merged_struts_binders_floppy_1988_C_BW_RP_LL_WD_MS_N_SS_V.csv')
...
#write to computer
write.csv(final_strut, 'prelim_clean/1988/merged_struts_binders_floppy_1988_C_BW_RP_LL_WD_MS_N_SS_V_SF.csv',
          row.names=FALSE)
