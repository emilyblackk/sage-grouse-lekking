#Emily Black
#Cleaning 1988 binder data - mud_springs
...
df_1 <- read.csv('raw_data/binders/mud_springs_1988_capture_binder_data.csv', 
                 header=FALSE)
...
previous_strut <- read.csv("prelim_clean/binder_capture_data_1988_prelim_CL_RP_LL_WD.csv") 
#Add hematoma air sac comments column
previous_strut <- previous_strut %>%
  mutate(hematomas_air_sac_comments = NA) %>%
  relocate(hematomas_air_sac_comments, .after = hematomas_air_sacs)
...
#Write to save
write.csv(final_strut,  "prelim_clean/binder_capture_data_1988_prelim_CL_RP_LL_WD_MS.csv", 
          row.names=FALSE)
