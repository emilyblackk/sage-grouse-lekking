#Emily Black
#Cleaning 1988 binder data - long_lake
...
df_1 <- read.csv('raw_data/binders/long_lake_1988_capture_binder_data.csv', 
                 header=FALSE)
...
unique(df_1$lek)
df_1 <- df_1 %>%
  mutate(lek_name = case_when(lek == "LONG LAKE " ~ "long_lake", 
...
previous_strut <- read.csv("prelim_clean/binder_capture_data_1988_prelim_CL_RP.csv") 
...
#Write to save
write.csv(final_strut,  "prelim_clean/binder_capture_data_1988_prelim_CL_RP_LL.csv", 
          row.names=FALSE)
