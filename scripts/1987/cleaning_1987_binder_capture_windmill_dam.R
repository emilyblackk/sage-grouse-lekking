...
df_1 <- read.csv('raw_data/binders/windmill_dam_1987_capture_binder_data.csv', 
                 header=FALSE)
...
previous_capture <- read.csv("prelim_clean/1987/binder_capture_data_1987_prelim_CL_BW_MS_MC_N_SF.csv")
...
#Write to github
 write.csv(full_capture, "prelim_clean/1987/binder_capture_data_1987_prelim_CL_BW_MS_MC_N_SF_WD.csv", 
           row.names=FALSE)


#small plot to check things worked
full_capture %>%
  ggplot(aes(x=as.numeric(num_lice_back_of_head), color = sex)) + 
  geom_histogram() +
  theme_classic()
