#Emily Black
#Cleaning 1987 strut data - windmill dam
...
df_1 <- read.csv('raw_data/binders/windmill_dam_1987_strut_binder_data.csv', 
                 header=FALSE)
...
unique(df_1$lek)
df_1 <- df_1 %>%
  mutate(lek_name = case_when(lek == "WINDMILL DAM LEK" ~ "windmill_dam")) %>%
...
previous_strut <- read.csv('prelim_clean/1987/merged_struts_binders_floppy_1987_C_BW_MS_N_SF.csv')
...
#write to computer
write.csv(final_strut, 'prelim_clean/1987/merged_struts_binders_floppy_1987_C_BW_MS_N_SF_WD.csv',
          row.names=FALSE)
