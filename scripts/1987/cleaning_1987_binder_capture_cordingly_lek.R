#Emily Black
#Cleaning 1987 binder data - cordingly
...
df_1 <- read.csv('raw_data/binders/cordingly_1987_capture_binder_data.csv', 
                 header=FALSE)
...
df_1 <- df_1 %>%
  mutate(age = case_when(age == 'adult' ~ 'adult', 
                         age=='yearling' ~ "juven")) #change the "juvenile" code in the data
...
#Write to github
write.csv(df_1, "prelim_clean/binder_capture_data_1987_prelim_CL.csv", 
          row.names=FALSE)
