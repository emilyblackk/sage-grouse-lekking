#Emily Black
#Cleaning 1987 strut data - cordingly, broken wing, mud springs
#note: these three were grouped because the raw data needed to
#be manually standardized to a format for cleaning
...
df_1 <- read.csv('raw_data/binders/cordingly_broken_wing_mud_springs_1987_strut_binder_data.csv', 
                 header=FALSE)
...
unique(df_1$lek)
df_1 <- df_1 %>%
  mutate(lek_name = case_when(lek == "CORDINGLY" ~ "cordingly", 
                              lek == "Broken Wing" ~ "broken_wing",
                              lek == "BROKEN WING" ~ "broken_wing",
                              lek == "MUD SPRINGS " ~ "mud_springs",
                              lek == "MUD SPRINGS" ~ "mud_springs")) %>%
...
leks <- c("mud_springs", 'cordingly', 'broken_wing')
...
table(df_wider$type)
#there are 73 struts from the floppy, 
...
#write to computer
write.csv(filtered_df_full_join, 'prelim_clean/1987/merged_struts_binders_floppy_1987_C_BW_MS.csv',
          row.names=FALSE)
