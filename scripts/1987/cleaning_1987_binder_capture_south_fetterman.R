#Emily Black
#Cleaning 1987 binder data - south fetterman
...
df_1 <- read.csv('raw_data/binders/south_fetterman_1987_capture_binder_data.csv', 
                 header=FALSE)
...
#rename cordingly
unique(df_1$lek)
df_1$lek_name <- c("south_fetterman")
df_1 <- df_1 %>%
...
previous_capture <- read.csv("prelim_clean/1987/binder_capture_data_1987_prelim_CL_BW_MS_MC_N.csv")
...
#Write to github
write.csv(full_capture, "prelim_clean/1987/binder_capture_data_1987_prelim_CL_BW_MS_MC_N_SF.csv", 
          row.names=FALSE)
