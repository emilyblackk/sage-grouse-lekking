#Emily Black
#Cleaning 1987 binder data - broken wing
...
df_1 <- read.csv('raw_data/binders/mud_springs_1987_capture_binder_data.csv', 
                 header=FALSE)
...
df_1 <- df_1 %>%
  mutate(samples_collected_blood_ = case_when(
    samples_collected_blood_=="Y" ~ "yes", 
    samples_collected_blood_=="N" ~ "no"
  )) %>%
...
df_1 <- df_1 %>%
  mutate(samples_collected_ectoparasites = case_when(
    samples_collected_ectoparasites=="Y" ~ "yes", 
    samples_collected_ectoparasites=="N" ~ "no"
  )) %>%
...
df_1 <- df_1 %>%
  mutate(samples_collected_fecal = case_when(
    samples_collected_fecal=="Y" ~ "yes", 
    samples_collected_fecal=="N" ~ "no"
  )) %>%
...
previous_capture <- read.csv("prelim_clean/1987/binder_capture_data_1987_prelim_CL_BW.csv")
head(previous_capture)


full_capture <- rbind(previous_capture, df_1)

#Write to github
write.csv(full_capture, "prelim_clean/1987/binder_capture_data_1987_prelim_CL_BW_MS.csv", 
          row.names=FALSE)
