#Emily Black
#Cleaning 1987 strut data - sybille springs
...
df_1 <- read.csv('raw_data/binders/sybille_springs_1987_strut_binder_data.csv', 
                 header=FALSE)
...
unique(df_1$lek)
df_1$lek_name <- "sybille_springs"
df_1 <- df_1 %>%
  relocate(lek_name, .after = lek) %>%
  select(-lek)
unique(df_1$lek_name)


#in the snowing column, replace dashes and "n" with "no" 
#and do the same for yes
df_1$snowing_status <- tolower(df_1$snowing_status)
df_1$snowing_status[df_1$snowing_status=="n"] <- "no"
df_1$snowing_status[df_1$snowing_status=="y"] <- "yes"
...
df_1$raining_status[df_1$raining_status=="-"] <- "no"
df_1$raining_status[df_1$raining_status=='n'] <- "no"
df_1$raining_status[df_1$raining_status=='y'] <- "yes"
...
df_1 <- df_1 %>%
  mutate(predator_disturbance_type_ = case_when(
    predator_disturbance_type_=="Raptor (golden eagle)" ~ "golden eagle",
    predator_disturbance_type_=="6 E" ~ "golden eagle", 
    TRUE ~ predator_disturbance_type_ 
  ))
df_1$predator_disturbance_type_ <- tolower(df_1$predator_disturbance_type_)
...
df_1 <- df_1 %>%
  rename(copulation_tag_colors_left = copulation_tag_colours_left) %>%
...
df_wider$date <- format(as.Date(df_wider$date, format = "%Y-%m-%d"), "%d-%m-%Y") 
...
#Get sturts for this year and lek - 1990 and sybille and mud springs
leks <- c("sybille_springs")
strut_filtered <- strut_floppy %>%
  filter(year == 1987, lek_name %in% leks) %>%
...
table(df_wider$type)
#there are 138 struts from the floppy, 
#and 146 in our dataframe
...
previous_strut <- read.csv('prelim_clean/1987/merged_struts_binders_floppy_1987_C_BW_MS_N_SF_WD.csv')

#merge the two

final_strut <- rbind(previous_strut, filtered_df_full_join)

#Last thing - replace NA NA with NA
final_strut[final_strut=="NA NA"] <- NA

#write to computer
write.csv(final_strut, 'prelim_clean/1987/merged_struts_binders_floppy_1987_C_BW_MS_N_SF_WD_SS.csv',
          row.names=FALSE)


#Test out - distance from hens vs strutting
bad_values <- c(0, 4)
df_filtered <- final_strut %>%
  filter(!is.na(distance_to_hens_category), !grepl(",", distance_to_hens_category), !distance_to_hens_category %in% bad_values)

df_filtered <- df_filtered %>%
  filter(!is.na(struts_5_min), !grepl(",", struts_5_min))

df_filtered %>%
  ggplot(aes(x=as.numeric(distance_to_hens_category), y=as.numeric(struts_5_min), color = breed)) +
  geom_point() + 
  theme_classic()


#For fun  - tie in with morphology
df_for_morph <- final_strut
df_for_morph$right_tag_code <- sapply(strsplit(df_for_morph$right_tag_code, ","), function(x) {
  if (length(x) > 1) {
    trimws(x[2])
  } else {
    x[1]
...
df_for_morph$right_tag_code <- trimws(df_for_morph$right_tag_code)

df_for_morph$right_tag_color <- sapply(strsplit(df_for_morph$right_tag_color, ","), function(x) {
  if (length(x) > 1) {
    trimws(x[2])
  } else {
    x[1]
...
# Remove leading and trailing whitespace
df_for_morph$right_tag_color <- trimws(df_for_morph$right_tag_color)

df_for_morph$left_tag_code <- sapply(strsplit(df_for_morph$left_tag_code, ","), function(x) {
  if (length(x) > 1) {
    trimws(x[2])
  } else {
    x[1]
...
# Remove leading and trailing whitespace
df_for_morph$left_tag_code <- trimws(df_for_morph$left_tag_code)

df_for_morph$left_tag_color <- sapply(strsplit(df_for_morph$left_tag_color, ","), function(x) {
  if (length(x) > 1) {
    trimws(x[2])
  } else {
    x[1]
...
# Remove leading and trailing whitespace
df_for_morph$left_tag_color <- trimws(df_for_morph$left_tag_color)

#create a summary strutting table
summary_struts <- df_for_morph %>%
  filter(!left_tag_color=="unmarked", 
         !left_tag_color=="NA", 
         !left_tag_color=="NA NA NA NA", 
         !right_tag_color=="NA") %>%
  group_by(lek_name, left_tag_color, right_tag_color) %>%
  dplyr::summarize(
    total_struts = sum(as.numeric(num_of_struts), na.rm = TRUE),
    avg_struts_5_min = mean(as.numeric(struts_5_min), na.rm = TRUE), 
    avg_distance_to_hens = mean(as.numeric(distance_to_hens_category)), 
    total_cops = sum(as.numeric(num_of_copulations), na.rm = TRUE)
  )

#Read in the morphology
capture_data <- read.csv('prelim_clean/1987/binder_capture_data_1987_prelim_CL_BW_MS_MC_N_SF_WD_SS.csv')

summary_struts_withmorph <- left_join(summary_struts, capture_data, by=c('lek_name', "left_tag_color", "right_tag_color"))



#Ok, first plot number of copulations against strut frequency, but adjust for distance to 
  summary_struts_withmorph  %>%
    ggplot(aes(x=as.character(hematomas_air_sacs), y=as.numeric(avg_struts_5_min))) + 
    geom_violin() + 
    theme_classic()
