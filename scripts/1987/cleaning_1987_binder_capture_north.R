#Emily Black
#Cleaning 1987 binder data - north lek
#Created: 10 Jul 2023
#Last modified: 
...
df_1 <- read.csv('raw_data/binders/north_lek_1987_capture_binder_data.csv', 
                 header=FALSE)
...
df_1 <- df_1 %>%
  mutate(comb_color = case_when(
    ...
    tolower(comb_color) == "yellow/gray" ~ "yellow gray",
    comb_color == "n/a" ~ NA,
    TRUE ~ comb_color
  ),
...
df_1 <- df_1 %>%
  mutate(air_sac_color = case_when(
    ...
    tolower(air_sac_color) == "yellow/green" ~ "yellow green",
    tolower(air_sac_color) == "yellow/gray" ~ "yellow gray",
...
previous_capture <- read.csv("prelim_clean/1987/binder_capture_data_1987_prelim_CL_BW_MS_MC.csv")
...
#Write to github
write.csv(full_capture, "prelim_clean/1987/binder_capture_data_1987_prelim_CL_BW_MS_MC_N.csv",
          row.names=FALSE)
