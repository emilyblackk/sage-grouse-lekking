...
df_1 <- read.csv('raw_data/binders/south_fetterman_1987_strut_binder_data.csv', 
                 header=FALSE)
...
unique(df_1$lek)
df_1 <- df_1 %>%
  mutate(lek_name = case_when(lek == "SOUTH FETTERMAN" ~ "south_fetterman")) %>%
...
#write to computer
write.csv(final_strut, 'prelim_clean/1987/merged_struts_binders_floppy_1987_C_BW_MS_N_SF.csv',
          row.names=FALSE)
