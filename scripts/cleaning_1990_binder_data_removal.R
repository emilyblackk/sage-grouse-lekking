#Emily Black
#Cleaning 1990 binder data - removal study
...
original_joined_df <- read.csv("prelim_clean/binder_capture_data_prelim_N_MS_SS.csv")

#read in the north lek data 
df_1 <- read.csv('raw_data/binders/removal_study_1990_capture_binder_data.csv', 
                 header=FALSE)
...
#NOTE: there are lots of inconsistencies in this dataset that need to be fixed
...
#note that the "removal" part of the study is indicated in the comments section, and the capture method
df_1 <- df_1 %>%
  mutate(lek_name = case_when(lek == "NORTH" ~ "north", 
                              lek == "SOUTH FETTERMAN" ~ "south_fetterman", 
                              lek == "BROKEN WING" ~ "broken_wing",
                              lek == "NORTH MARSHALL" ~ "north_marshall", ) ) %>%
...
#The weight bird and bag column has some weights in pounds
#Need to make those consistently kilograms 
remove_non_numeric <- function(x) {
  gsub("[^0-9.]", "", x)
}

# Function to convert pounds to kilograms
convert_to_kg <- function(x) {
  ifelse(x > 5, x * 0.45359237, x)
}

df_1$weight_bird__bag_g_ <- remove_non_numeric(df_1$weight_bird__bag_g_)
df_1$weight_bird__bag_g_ <- convert_to_kg(as.numeric(df_1$weight_bird__bag_g_))

df_1$bird_weight_g_ <- remove_non_numeric(df_1$bird_weight_g_)
df_1$bird_weight_g_ <- convert_to_kg(as.numeric(df_1$bird_weight_g_))

#given that they didn't use a bag for capture (since the birds are dead), we can say that 
#the weight of the bird + bag is the same as the weight of the bird, and just duplicate the column
df_1$bird_weight_g_ <- df_1$weight_bird__bag_g_

#rename some columns
df_1 <- df_1 %>%
  rename(bird_weight_g = bird_weight_g_) %>%
  rename(weight_bird_and_bag = weight_bird__bag_g_) %>%
  rename(weight_bag_g = weight_bag_g_)


#Now, we had a lot of cells with inches instead of mm 
#I did try to write some code for this, but none of it worked due to the structure of the df
#Had to correct manually 
...
unique(df_1$air_sac_color_)
df_1 <- df_1 %>%
  rename(air_sac_color = air_sac_color_) %>%
...
#This one is unique because some values just say "too late"
#These will be replaced with NA, since we don't know 

split_values <- strsplit(df_1$num_lice_on_air_sac_, ",")
...
df_1 <- df_1 %>%
  relocate(num_lice_on_air_sac_left, .after = num_lice_on_air_sac_) %>%
  ...
  select(-num_lice_on_air_sac_)


#Lice on back of head has a variety of values
#Standardize these as best as we can 
df_1 <- df_1 %>%
  mutate(lice_back_of_head = case_when(
    lice_on_back_of_head__=="+" ~ "yes", 
    lice_on_back_of_head__== "0" ~ "no", 
    TRUE ~ lice_on_back_of_head__)) %>%
  relocate(lice_back_of_head, .after = lice_on_back_of_head__) %>%
  select(-lice_on_back_of_head__)

#Looks like num_lice_on_comb needs some fixing too 
df_1 <- df_1 %>%
  mutate(num_lice_on_comb = case_when(
    tolower(num_lice_on_comb_)=='none' ~ "0", 
    tolower(num_lice_on_comb_)=="no" ~ "0", 
    num_lice_on_comb_== "-" ~ NA, 
    num_lice_on_comb_== 'n/a' ~ NA, 
    TRUE ~ num_lice_on_comb_)) %>%
  relocate(num_lice_on_comb, .after = num_lice_on_comb_) %>%
  select(-num_lice_on_comb_)
...
#The code below splits them into two columns - air sacs 
# Split the values and assign them to the new columns
#But also, for the removal study, there are lots of comments
#So I will rename the original column rather than deleting it 
df_1 <- df_1 %>%
  rename(hematomas_num_air_sacs = `_hematomas_air_sacs_`)
...
#need to case the "none" values into zero 
df_1$hematomas_num_combs <- as.character(df_1$hematomas_num_combs)
df_1 <- df_1 %>%
  mutate(hematomas_num_combs = case_when(
    hematomas_num_combs == "None" ~ "0", 
    TRUE ~ hematomas_num_combs
  ))
...
df_1 <- df_1 %>%
  ...
  rename(left_tag_number = numbers_left_wing) %>%
...
df_1 <- df_1 %>%
  ...
  rename(right_tag_number = numbers_right_wing)%>%
...
#Replace some last dashes and spaces with NA
df_1[df_1 =="-"] <- NA
df_1[df_1 ==""] <- NA
...
df_2 <- read.csv('raw_data/binders/removal_study_1990_capture_lab_binder_data.csv', 
                 header=FALSE)
...
df_2 <- df_2 %>%
  mutate(lek_name = case_when(lek_ == "NORTH" ~ "north", 
                              lek_ == "SOUTH FETTERMAN" ~ "south_fetterman", 
                              lek_ == "BROKEN WING" ~ "broken_wing",
                              lek_ == "N. MARSHALL" ~ "north_marshall", ) ) %>%
