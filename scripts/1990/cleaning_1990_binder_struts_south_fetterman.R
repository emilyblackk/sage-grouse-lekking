...
df_1 <- read.csv('raw_data/binders/south_fetterman_1990_strut_binder_data.csv', 
                 header=FALSE)
...
df_1 <- df_1 %>%
  ...
  rename(copulation_tag_colors_right = copulation_tag_colours_right) %>%
...
#Add columns with NAs to match the previous strutting files
df_wider$breed <- NA
df_wider$malaria <- NA
df_wider$lice <- NA
df_wider$age <- NA
...
#dobule check there isn't any south fetterman struts from the floppies
strut_floppy <- read.csv('prelim_clean/strut_merged_df_prelim.csv')
leks <- unique(filtered_df_full_join$lek_name)
south_fetterman <- strut_floppy %>%
  filter(lek_name %in% leks, year=="1990")
#nope, we're good!

#Merge this data with the others
#Open the previous strut dataset
previous_strut <- read.csv('prelim_clean/1990/merged_struts_binders_floppy_MS_N.csv')

#merge the two 
final_strut <- rbind(previous_strut, filtered_df_full_join)

#write to computer
write.csv(final_strut, 'prelim_clean/1990/merged_struts_binders_floppy_MS_N_SF.csv', 
          row.names=FALSE)
